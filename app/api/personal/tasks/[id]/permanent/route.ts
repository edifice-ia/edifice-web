...
// SUPPRESSION PHYSIQUE ET IRREVERSIBLE d'une tache archivee.
//
...
//   3. filtre triple cote store : id + user_id + deleted_at non nul. Une tache
//      ACTIVE ne peut pas etre supprimee par cette route, meme en postant son
//      id ;
//   4. execution par la cle service-role, seule capable de supprimer :
//      personal_tasks n'accorde pas DELETE a authenticated et ne porte aucune
//      policy DELETE.
//
// Ce geste ne vide pas le module : les autres taches archivees ne sont pas
// touchees. Voir /api/personal/settings/erase pour le geste a l'echelle du
// module.
//
// Aucun store propre a Taches n'intervient : permanentlyDeletePersonalItem est
// generique sur ErasableModuleId, et Taches y est declare depuis l'extension du
// geste module. C'est le meme invariant que partout dans le pole — un seul
// fichier du depot supprime physiquement des donnees Personnel.
export async function DELETE(
  ...
  try {
    ...
    // null si la tache n'existe pas, ne lui appartient pas, ou n'est pas
    // archivee. Les trois repondent 404 sans se distinguer : les separer
