...
export const CALENDAR_STATE_COOKIE = "edifice_calendar_oauth_state";
...
export function verifyCalendarOAuthState(receivedState: string | null, userId: string) {
  const secret = getStateSecret();
  ...
  if (ageSeconds < 0 || ageSeconds > CALENDAR_STATE_MAX_AGE_SECONDS) {
    return false;
  }
