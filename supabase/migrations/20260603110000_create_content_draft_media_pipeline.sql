create table if not exists public.content_draft_media_plans (
  draft_id uuid primary key references public.content_drafts(id) on delete cascade,
  media_pipeline_status text not null default 'draft' check (
    media_pipeline_status in (
      'draft',
      'validated',
      'media_preparing',
      'media_ready',
  ...
  visual_decision jsonb not null default '{}'::jsonb,
...
create table if not exists public.content_draft_asset_links (
  ...
  asset_source text not null check (asset_source in ('library', 'generated')),
  score numeric(5, 2) not null default 0,
  usage_order integer not null check (usage_order > 0),
  created_at timestamptz not null default now(),
  unique (draft_id, usage_order)
);
